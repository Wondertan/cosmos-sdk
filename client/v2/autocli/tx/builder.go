...
func (w *wrapper) GetMsgsV2() ([]proto.Message, error) {
	if w.msgsV2 == nil {
...
func (w *wrapper) getBodyBytes() []byte {
	if len(w.bodyBz) == 0 {
		...
		w.bodyBz, err = proto.Marshal(&txv1beta1.TxBody{
			Messages:      anyMsgs,
			Memo:          w.tx.Body.Memo,
			TimeoutHeight: w.tx.Body.TimeoutHeight,
		})
		if err != nil {
			panic(err)
		}
...
func (w *wrapper) getAuthInfoBytes() []byte {
	if len(w.authInfoBz) == 0 {
		...
		// transmitted over the wire
		var err error
		w.authInfoBz, err = gogoproto.Marshal(w.tx.AuthInfo)
		if err != nil {
			panic(err)
		}
	}
